...
""".strip()
        ...
        try:
            resp = self.client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    ...
                    response_schema=NoteSummary,  # ✅ Pydantic schema (cara yang disarankan SDK) :contentReference[oaicite:3]{index=3}
                ),
            )
        except Exception as e:
            raise NoteSummaryAPIError("Failed to call Gemini API.") from e

        # ✅ ideal: pakai parsed dari SDK
        data = getattr(resp, "parsed", None)
        if isinstance(data, NoteSummary):
            data = data.model_dump()

        # fallback kalau parsed None
        if data is None:
            try:
                data = json.loads(
        ...
        # enforce bullets jadi tags 1–2 kata
        tags, seen = [], set()
        for b in bullets:
